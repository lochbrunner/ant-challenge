...
impl SimpleMesh {
    ...
    pub fn mesh(gl: &GL, mesh_name: &str, texture: &str) -> SimpleMesh {
        let meshes = include_bytes!("../../models/meshes.bin");
        ...
        if meshes.is_empty() {
            panic!("Meshes file does not contain any mesh.");
        }
        if !meshes.contains_key(mesh_name) {
            let keys = meshes.keys().cloned().collect::<Vec<_>>().join(", ");
            panic!(
                "Can not find mesh with name \"{}\". Available are {}",
                mesh_name, keys
            );
        }
